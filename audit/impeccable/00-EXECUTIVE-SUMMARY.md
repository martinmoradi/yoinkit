...
> Audited: 2026-06-18. Five subsystem deep-dives live in [`reports/`](reports/), each fanning out into focused sub-dives (e.g. `03a`–`03f`); the YoinkIt payoff is in [`PATTERNS-FOR-YOINKIT.md`](PATTERNS-FOR-YOINKIT.md).

## What Impeccable is

On the surface, Impeccable is a design-quality tool for AI coding agents: one `/impeccable` skill exposing 23 sub-commands (`polish`, `audit`, `critique`, `animate`, ...), plus a standalone CLI and Chrome extension that run 44 deterministic anti-pattern rules with no LLM and no API key. From a single source it compiles to 13 provider targets (12 committed install directories), spanning about a dozen agent harnesses (Claude Code, Cursor, Codex, Gemini, GitHub Copilot, Kiro, OpenCode, Pi, Qoder, Trae, Trae CN, Rovo Dev).
...
flowchart TB
    subgraph L1["LAYER 1: Understanding engine (deterministic, no LLM)"]
        ...
        CORE --> A1["hand-rolled CSS cascade<br/>(static HTML, no browser, no jsdom)"]
        CORE --> A2["Puppeteer live page<br/>(headless URL scan)"]
        CORE --> A3["Injected in-page bundle<br/>(generated by concatenation)"]
        CORE --> A4["Regex on source text<br/>(re-implements the decision; can drift)"]
        CORE --> A5["Screenshot pixel-diff<br/>(visual tier; one rule)"]
    ...
    subgraph L3["LAYER 3: Async human↔agent collaboration"]
        SKILL["One SKILL.src.md<br/>compiled to 13 provider targets"]
...
**Layer 1: the understanding engine.** A deterministic, no-LLM engine inspects arbitrary pages and emits structured *findings*, never code. The same 44 rule cores run across four detection engines — regex over raw source, a hand-rolled CSS cascade for static HTML (no browser, no jsdom), a live browser, and a screenshot pixel-diff tier for the cases computed style cannot decide. The live-browser engine ships two ways — Puppeteer for headless pages and an injected in-page bundle for the extension and overlay — and that bundle is *generated by concatenation* from the same ES modules, so the live detector and the Node detector literally share code and cannot drift (only the regex tier re-implements the decision independently, so it is the one place that could). See [report 01a](reports/01-detector-engine/01a-rule-trinity-and-dispatch.md) for the full rule×engine matrix. There is also a `design-system.mjs` that extracts tokens, type, and color from any page. This is the direct analog of YoinkIt's capture engine: framework-agnostic, dependency-free, injected into third-party pages, emitting a spec.
...
A separate but equally relevant achievement sits beside live mode: the **single-source skill distribution**. One `SKILL.src.md` plus a `reference/` tree compiles to 13 provider targets (12 of which are committed install directories) through a ~170-line transformer core (`createTransformer`, `factory.js:156-326`) over two config tables. YoinkIt currently hand-maintains parallel `skill/codex/` and `skill/claude/` copies. This is the pattern that dissolves that duplication.
...
flowchart LR
    subgraph IMP["IMPECCABLE"]
        ...
        I6["One SKILL.src.md → 13 provider targets"]
...
- **Impeccable writes code into the user's real source and leans on HMR.** Its agent owns the repo and the dev server. At the ADR's altitude "accept a variant" reduces to "delete the losing variants from the file," but the *shipped* accept is two-phase: a fast inline draft write for instant feedback, then a gated "carbonize" cleanup that refuses to complete until the agent rewrites it clean (see pattern B9). YoinkIt emits a spec and deliberately refuses to write code, so the file-writing does not transfer. What transfers is the *state machine* around it (instant draft, gated finalize), not the code-writing.
