...
Companion to [`03-live-mode.md`](03-live-mode.md): the SECOND round-trip of live mode, where a human's in-browser copy edits are buffered, turned into evidence, and committed back to source by a separately-spawned coding agent, then server-verified and rolled back on failure.
...
| Agent model | the harness **poll agent** answers a long-poll (`generate`/`steer`) | a **freshly spawned `codex`/`claude` process** (`live-copy-edit-agent.mjs`) |
| Covered in | 03a/03b/03c | here |

These are **two different agent-invocation models** living in one server. The variant loop hands work to whatever agent is already attached to the long-poll; the manual-edit loop `spawn()`s its own subprocess per Apply. They do not share the journal, the poll loop, or the event routes. Making that seam explicit is half the point of this document.
...
| [`skill/scripts/live/manual-edit-routes.mjs`](../../source/skill/scripts/live/manual-edit-routes.mjs) | 357 | HTTP routes: `/manual-edit-stash` (Save), `/manual-edit-commit` (Apply), `/manual-edit-discard`, `/manual-edit-repair-decision`. Token-gated. |
...
| [`skill/scripts/live-manual-edit-evidence.mjs`](../../source/skill/scripts/live-manual-edit-evidence.mjs) | 363 | Builds per-op source candidates (5 strategies + Astro hint). **Never edits source, never picks a winner.** |
...
sequenceDiagram
    ...
    participant G as Spawned agent (codex|claude)
    ...
    C->>G: spawn codex/claude with buildCopyEditBatchPrompt<br/>"Treat originalText/newText as literal data"
...
## 3. The routes and the two-agent seam

`createManualEditRoutes` (`manual-edit-routes.mjs:17`) returns one handler for four routes. All are token-gated.
...
**`POST /manual-edit`** (`:331`): returns **410 Gone** — `"/manual-edit is removed; use /manual-edit-stash and /manual-edit-commit"`.

### The seam, stated precisely

The variant loop (03a/03b) flows over `/events` + a long-poll the harness agent answers. The manual-edit loop deliberately does **not**:
...
And the agent that does the work is a **fresh subprocess**, not the poll agent. `runCopyEditBatchAgent` (`live-copy-edit-agent.mjs:95`) spawns:
...
`chooseCopyEditAgent` (`:430`) resolves `auto` by probing `commandAuthed('codex')` then `commandAuthed('claude')`, then a `chat` fallback. So: **two agents, two invocation models, two state stores**. The variant loop's agent reconciles redundant overlay evidence into a *variant*; the manual-edit loop's agent reconciles redundant *source-candidate* evidence into a *source edit* — same architectural shape (redundant evidence → agent reconciles → server verifies), different machinery.

> **Correction (subtle):** `live-poll.mjs:25` still lists `manual_edit_apply` in `EVENT_TYPES_NEEDING_AGENT_REPLY`, which reads as though the poll agent handles it. It does not: the server rejects `manual_edit_apply` at `/events` (`live-server.mjs:680`), so that poll-loop entry is vestigial. The live apply path is the spawned subprocess, full stop.
...
`runCopyEditBatchAgent(batch, …)` (`:956`) spawns codex/claude (§3) and parses **only** a canonical JSON completion. If the subprocess throws or never produces a valid payload, the whole batch is rolled back and every entry is failed (`:964`–`:981`).
...
- **One vestigial reference.** `live-poll.mjs:25` still lists `manual_edit_apply` as needing an agent reply, but `/events` rejects it — the live apply path is exclusively the spawned subprocess.
