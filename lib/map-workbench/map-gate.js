...
function motionDiscoveryBlockers(motionCandidatesArtifact, config = {}, pageModel = {}) {
  ...
  const requiredSources = discovery && Array.isArray(discovery.requiredSources)
    ? discovery.requiredSources.map(item => String(item || '').trim()).filter(Boolean)
    : [];
  const inspections = discovery && Array.isArray(discovery.inspections) ? discovery.inspections : null;
  if (!requiredSources.length || !inspections) return null;
  ...
  const blockers = [];
  for (const viewportId of configuredViewportIds(config, pageModel)) {
