...
function approveException(pageModel, decision, approvedAt) {
  ...
  if (existing && existing.stage !== 'map-gate') {
    throw new Error(`map-gate --approve-exception id "${decision.exceptionId}" already exists for stage ${existing.stage || 'unknown'}; choose a map-gate exception id`);
  }
