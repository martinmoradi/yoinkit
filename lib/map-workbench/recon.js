...
function readinessStatus(raw, blockers) {
  ...
  if (blockers.some(blocker => REAL_BLOCKER_TYPES.has(blocker.type))) return 'blocked';
...
function runRecon(runDir, env = {}) {
  ...
  for (const viewport of orderedViewports(config)) {
    ...
    const raw = driver.probe(config.targetUrl, viewport, { runDir: absRunDir, config });
