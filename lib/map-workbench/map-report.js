...
function reportRelativePath(reportDir, runRelative) {
  return path.relative(reportDir, path.join(path.dirname(reportDir), runRelative)).split(path.sep).join('/');
}
...
function renderSourceMode({ pageModel, reportDir, runDir, viewportId }) {
  ...
  return `
    <section class="report-mode source-mode" data-mode="source" aria-labelledby="source-mode-title">
...
function renderStyles() {
  return `<style>
    ...
    * { box-sizing: border-box; }
    ...
    nav a { color: var(--ink); text-decoration: none; border: 1px solid var(--line); padding: 6px 10px; border-radius: 6px; background: white; font-size: 13px; }
...
<body>
  <header>
    ...
    <nav aria-label="Report modes">
      <a href="#source-mode-title">Source</a>
      <a href="#region-mode-title">Region</a>
      <a href="#gate-mode-title">Gate</a>
    </nav>
  </header>
  <main>
