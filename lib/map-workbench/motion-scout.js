...
function normalizeInspection(input = {}, source, viewportId) {
  ...
  const evidence = evidenceText(input, status === 'complete' ? `${source} inspected` : '');
  const syntheticEvidence = `${source} inspected`;
  return {
    source,
    viewportId,
    ...
    status,
    candidates: 0,
    evidence: status === 'missing' && evidence === syntheticEvidence ? '' : evidence,
