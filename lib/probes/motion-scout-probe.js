(function () {
  ...
  function runMotionScoutProbe() {
    ...
    // These four sources are inspect-by-running DOM scans: they report
    // `complete` unless their function throws. Only the five __cap.map() sources
    // above fail-closed (`missing`) when the engine omits a field, so that is
    // where the discovery matrix's real blocking power lives.
